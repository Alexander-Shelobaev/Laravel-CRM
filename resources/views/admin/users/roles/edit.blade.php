...
<ol class="breadcrumb pull-right">
	...
	<li class="breadcrumb-item active"><a href="/admin">Редактирование роли</a></li>
</ol>

<h1 class="page-title">Редактирование роли</h1>

<form action="{{ route('roles.update', $value) }}" method="post" enctype="multipart/form-data">
	...
	<div class="form-group">
		<label for="code">Код <span class="text-theme">*</span> <br>
			<small>Текст длиною не более 100 символов</small>
		</label>
		<input id="code" name="code" type="text" class="form-control{{ $errors->has('code') ? ' is-invalid' : '' }}" value="@if(old('code')){{ old('code') }}@else{{$value->code ?? ''}}@endif" autofocus><!-- удалить!!!!!!!!!! required -->
		@if ($errors->has('code'))
		<span class="invalid-feedback" role="alert">
	...
	<div class="form-group row m-b-10">
		<label class="col-md-3 col-form-label">Доступы</label>
		<div class="col-md-9">
			@foreach ($permissions as $permission)
				<div class="checkbox checkbox-css">
					<input type="checkbox" id="{{ $permission->id }}" name="checkbox-permissions[{{ $permission->id }}]" value="{{ $permission->name }}" 
						@foreach ($value->permissions as $acticePer)
							@if ($permission->id == $acticePer->id)
								checked
							@endif
						@endforeach
					> <!--checked=""-->
