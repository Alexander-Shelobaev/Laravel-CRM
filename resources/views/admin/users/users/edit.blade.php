...
<form action="{{ route('users.update', $value) }}" method="post" enctype="multipart/form-data">
		...
		<div class="col-md-9">
			@foreach ($roles as $role)
				<div class="checkbox checkbox-css">
					<input type="checkbox" id="{{ $role->id }}" name="checkbox-roles[{{ $role->id }}]" value="{{ $role->name }}" 
	...
	<div class="form-group">
		<label for="old_avatar">Текущее изображение анонса <!-- под названием: {{ $value->picture_announce }}--></label>
		<input id="old_avatar" name="old_avatar" type="hidden" value="{{ $value->avatar }}">
	</div>

	<img class="old_picture_announce height-30" src="{{asset('/assets/img/user/')}}/{{ $value->avatar }}" alt="{{ $value->avatar }}" />
