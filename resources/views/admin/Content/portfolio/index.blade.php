...
<tr class='gradeA'>
	...
	<td>{{ $value->href }}</td>
	<td class="with-img picture-announce"><img src="{{ asset('/assets-landing/img/'.$value->img) }}" class="img-rounded picture-announce"></td>
	<td class="with-btn" nowrap="">
		<a class="btn btn-success" href="{{ route('portfolios.edit',$value) }}"><i class="fas fa-edit"></i> Редактировать</a>
		<form onsubmit="if(confirm('Удалить?')){ return true } else { return false }" action="{{ route('portfolios.destroy',[$value->id]) }}" method="post" class="delete-btn" >
			{{ method_field('delete') }} 
			{{ csrf_field() }} 
...
<div class="container-fluid">
	...
	<br><p><a class="btn btn-lime" href="{{ route('portfolios.create') }}"><i class="fas fa-plus-circle"></i> Добавить</a></p>
