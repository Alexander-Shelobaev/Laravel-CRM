    ...
    <div class="panel-body">
        ...
        <form action="{{ route('news.update', $value) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                value="@if(old('headline')){{ old('headline') }}@else{{$value->headline ?? ''}}@endif" autofocus>
                <!-- required -->
            ...
            <div class="form-group">
                ...
                value="@if(old('alias')){{ old('alias') }}@else{{$value->alias ?? ''}}@endif">
                @if ($errors->has('alias'))
                <span class="invalid-feedback" role="alert">
            ...
            <div class="form-group">
                ...
                <input id="desc_announce" name="desc_announce" type="text" 
                class="form-control{{ $errors->has('desc_announce') ? ' is-invalid' : '' }}" 
                value="@if(old('desc_announce')){{ old('desc_announce') }}
                       @else{{$value->desc_announce ?? ''}}@endif" autofocus> <!-- required -->
            ...
            <div class="form-group">
                <label for="old_picture_announce">
                    Текущее изображение анонса <!-- под названием: {{ $value->picture_announce }}-->
                </label>
                <input id="old_picture_announce" name="old_picture_announce" type="hidden" 
                value="{{ $value->picture_announce }}">
            </div>

            <img class="old_picture_announce" 
            ...
            <div class="form-group">
                ...
                <div id="picture_announce" name="picture_announce" class="input-group input-file-picture-announce">
                    <input type="text" class="form-control{{ $errors->has('picture_announce') ? ' is-invalid' : '' }}" 
            ...
            <div class="form-group">
                <label for="old_detailed_picture">
                    Текущее изображение для детального описания <!--под названием: {{ $value->detailed_picture }}-->
                </label>
                <input id="old_detailed_picture" name="old_detailed_picture" type="hidden" 
                value="{{ $value->detailed_picture }}">
            </div>
            <img class="old_detailed_picture" src="{{asset('/assets-landing/img')}}/{{ $value->detailed_picture }}" 
            alt="{{ $value->headline }}" />
            <div class="form-group">
                ...
                <div id="detailed_picture" name="detailed_picture" class="input-group input-file-detailed-picture">
                    <input type="text" class="form-control{{ $errors->has('detailed_picture') ? ' is-invalid' : '' }}"
                    placeholder='Выберите файл' />
                    <div class="input-group-append">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control{{ $errors->has('detailed_desc') ? ' is-invalid' : '' }}" 
                id="detailed_desc" name="detailed_desc" rows="12" >
                    @if(old('detailed_desc')){{ old('detailed_desc') }}@else{{$value->detailed_desc ?? ''}}@endif
                </textarea>
            ...
            <div class="form-group">
                ...
                placeholder="Пример: 2000-12-31 23:59:59" 
                value="@if(old('created_at')){{ old('created_at') }}@else{{$value->created_at ?? ''}}@endif">
