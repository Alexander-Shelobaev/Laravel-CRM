...
<div class="panel panel-inverse">
    <div class="panel-body">
        ...
        <form action="{{ route('states.update', $value) }}" method="post" enctype="multipart/form-data">

            <div class="form-group">
                <label for="name_state_en">
                    Название валюты на английском языке <span class="text-theme">*</span><br>
                    <small>Текст длиною не более 255 символов</small>
                </label>
                <input class="form-control{{ $errors->has('name_state_en') ? ' is-invalid' : '' }}"
                id="name_state_en" name="name_state_en" type="text"
                value="@if(old('name_state_en')){{ old('name_state_en') }}@else{{$value->name_state_en ?? ''}}@endif"
                autofocus>
            ...
            <div class="form-group">
                <label for="name_state_ru">
                    Название валюты на русском языке <span class="text-theme">*</span><br>
                    <small>Текст длиною не более 255 символов</small>
                </label>
                <input class="form-control{{ $errors->has('name_state_ru') ? ' is-invalid' : '' }}"
                id="name_state_ru" name="name_state_ru" type="text"
                value="@if(old('name_state_ru')){{ old('name_state_ru') }}@else{{$value->name_state_ru ?? ''}}@endif"
                autofocus>
            ...
            <div class="form-group">
                <label for="iso_code_3_state">
                    Код ISO 4217 буквенный для данной валюты <span class="text-theme">*</span><br>
                    <small>Текст длиною не более 3 символов</small>
                </label>
                <input class="form-control{{ $errors->has('iso_code_3_state') ? ' is-invalid' : '' }}"
                id="iso_code_3_state" name="iso_code_3_state" type="text"
                value="@if(old('iso_code_3_state'))
                {{ old('iso_code_3_state') }}@else{{$value->iso_code_3_state ?? ''}}
                @endif" autofocus>
            ...
            <div class="form-group">
                <label for="iso_code_2_state">
                    Код ISO 4217 цифровой для данной валюты <span class="text-theme">*</span><br>
                    <small>Текст длиною не более 2 символов</small>
                </label>
                <input class="form-control{{ $errors->has('iso_code_2_state') ? ' is-invalid' : '' }}"
                id="iso_code_2_state" name="iso_code_2_state" type="text"
                value="@if(old('iso_code_2_state')){{ old('iso_code_2_state') }}@else{{$value->iso_code_2_state ?? ''}}@endif"
                autofocus>
            ...
            <div class="form-group">
                <label for="currency_id">
                    Величина (кол-во знаков после запятой), до которой округляется валюты<br>
                    <small>Число, значением не более 999</small>
                </label>
                <input class="form-control{{ $errors->has('currency_id') ? ' is-invalid' : '' }}"
                id="currency_id" name="currency_id" type="number"
                value="@if(old('currency_id')){{ old('currency_id') }}@else{{$value->currency_id ?? ''}}@endif"
                autofocus>
            ...
            <div class="form-group">
                <label for="solid_currency_id">
                    Знак валюты в формате юникод  <br>
                    <small>Число, значением не более 999</small>
                </label>
                <input class="form-control{{ $errors->has('solid_currency_id') ? ' is-invalid' : '' }}"
                id="solid_currency_id" name="solid_currency_id" type="number"
                value="@if(old('solid_currency_id'))
                {{ old('solid_currency_id') }}@else{{$value->solid_currency_id ?? ''}}
                @endif" autofocus>
