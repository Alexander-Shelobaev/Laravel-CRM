    ...
    <div class="panel-body">
        ...
        <form action="{{ route('cities.store') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name_city_en">
                    Название города на английском языке <span class="text-theme">*</span><br>
                    <small>Текст длиною не более 255 символов</small>
                </label>
                <input class="form-control{{ $errors->has('name_city_en') ? ' is-invalid' : '' }}" id="name_city_en"
                name="name_city_en" type="text"
                value="@if(old('name_city_en')){{ old('name_city_en') }}@else{{$value->name_city_en ?? ''}}@endif"
                autofocus>
            ...
            <div class="form-group">
                <label for="name_city_ru">
                    Название города на русском языке <span class="text-theme">*</span><br>
                    <small>Текст длиною не более 255 символов</small>
                </label>
                <input class="form-control{{ $errors->has('name_city_ru') ? ' is-invalid' : '' }}" id="name_city_ru"
                 name="name_city_ru" type="text"
                 value="@if(old('name_city_ru')){{ old('name_city_ru') }}@else{{$value->name_city_ru ?? ''}}@endif"
                 autofocus>
            ...
            <div class="form-group">
                <label for="iata_code_city">
                    IATA код города <span class="text-theme">*</span><br>
                    <small>Текст длиною не более 4 символов</small>
                </label>
                <input class="form-control{{ $errors->has('iata_code_city') ? ' is-invalid' : '' }}"
                 id="iata_code_city" name="iata_code_city" type="text"
                 value="@if(old('iata_code_city')){{ old('iata_code_city') }}@else{{$value->iata_code_city ?? ''}}@endif"
                 autofocus>
            ...
            <div class="form-group">
                <label for="state_id">
                    Ссылка на государство, в котором расположен город<br>
                    <small>Число, значением не более 999</small>
                </label>
                <input class="form-control{{ $errors->has('state_id') ? ' is-invalid' : '' }}" id="state_id"
                 name="state_id" type="number"
                 value="@if(old('state_id')){{ old('state_id') }}@else{{$value->state_id ?? ''}}@endif" autofocus>
