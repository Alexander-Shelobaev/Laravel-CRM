...
class CreateAirfieldsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('airfields', function (Blueprint $table) {
            ...
            $table->string('name_airfield_en', 255)
                ->comment('Название авиаплощадоки на английском языке');
            $table->string('name_airfield_ru', 255)->nullable()
                ->comment('Название авиаплощадки на русском языке');
            $table->string('iata_code_airfield', 4)
                ->comment('IATA код площадки');
            // колонка необходима для создание связи, позже должна быть удалена
            $table->string('iata_code_city', 4)->nullable()
                ->comment('IATA код города, в котором расположена авиаплощадки');   
            $table->string('latitude', 16)->nullable()
                ->comment('Широта');
            $table->string('longitude', 16)->nullable()
                ->comment('Долгота');
            $table->string('time_zone', 16)->nullable()
                ->comment('Часовой пояс, в котором расположен авиаплощадки');
            $table->bigInteger('city_id')->unsigned()->nullable()
                ->comment('Ссылка на город, в котором расположена авиаплощадки');
            $table->timestamps();
        });
